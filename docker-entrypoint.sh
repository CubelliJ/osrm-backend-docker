...
PORT=${PORT:=8080}
#OSRM_PBF_URL=${OSRM_PBF_URL:="http://download.geofabrik.de/south-america/chile-latest.osm.pbf"}
...
osrm-routed /osrm-data/data.osrm --port $PORT --max-table-size $OSRM_MAX_TABLE_SIZE &
child=$!
wait "$child"

####

#curl -L "http://download.geofabrik.de/south-america/chile-latest.osm.pbf" --create-dirs -o /osrm-data/data.osm.pbf

#osrm-extract /osrm-data/data.osm.pbf -p /osrm-profiles/car.lua

#osrm-contract /osrm-data/data.osm.pbf